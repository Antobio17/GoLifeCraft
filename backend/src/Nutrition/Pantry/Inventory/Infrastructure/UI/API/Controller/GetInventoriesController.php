...
use Nutrition\Pantry\Inventory\Application\Query\GetInventoriesQuery;
...
final class GetInventoriesController
{
    use HandleTrait;
    ...
    public function __invoke(Request $request): JsonResponse
    {
        try {
            return JsonResponseBuilder::buildCollectionResponse(
                queryCollectionResult: $this->handle(message: new GetInventoriesQuery(
                    pageNumber: RequestExtractor::getPageNumber(request: $request),
                    pageSize: RequestExtractor::getPageSize(request: $request),
                    filterShift: RequestExtractor::getFilterParam(request: $request, filterName: 'shift'),
