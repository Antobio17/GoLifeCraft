...
final readonly class DoctrineGetMenuShoppingNeedsNeedleDataQuery implements GetMenuShoppingNeedsNeedleDataQuery
{
    ...
    private function buildNeeds(array $quantities): array
    {
        ...
        $rows = $this->connection->createQueryBuilder()
            ->select('a.id', 'a.name', 'a.emoji', 'a.brand', 'a.base_unit', 's.name AS store', 'sli.id AS shopping_list_item_id')
        ...
        return array_map(
            callback: static fn (array $row): MenuShoppingNeedView => new MenuShoppingNeedView(
                articleId: $row['id'],
                name: $row['name'],
                emoji: $row['emoji'] ?? '🥫',
                brand: $row['brand'],
