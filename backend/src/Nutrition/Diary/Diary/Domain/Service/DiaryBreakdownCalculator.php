...
final class DiaryBreakdownCalculator
{
    ...
    {
        $byDepth = $items;
        usort($byDepth, static fn (DiaryBreakdownItem $left, DiaryBreakdownItem $right): int => $right->depth <=> $left->depth);
        ...
        return array_map(static fn (DiaryBreakdownItem $item): DiaryBreakdownItem => $refreshed[$item->path], $items);
    }
    ...
    private function expandRecipe(
        ...
        foreach (array_values($graph->recipeIngredients(recipeId: $recipeId)) as $position => $ingredient) {
            $path = DiaryEntryNode::buildPath(parentPath: $parentPath, position: $position);
            $quantity = DiaryEntryNode::round(value: $ingredient['quantity'] * $factor);

            if (DiaryEntryNode::KIND_PRODUCT === $ingredient['kind']) {
    ...
    private function refreshProductItem(RecipeNutritionGraph $graph, DiaryBreakdownItem $item): DiaryBreakdownItem
    {
    ...
    /** @param array<string, DiaryBreakdownItem> $refreshed */
    private function refreshRecipeItem(RecipeNutritionGraph $graph, DiaryBreakdownItem $item, array $refreshed): DiaryBreakdownItem
    {
        return $item->withSnapshot(
