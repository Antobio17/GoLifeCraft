...
final class RestoreProductionItemIngredientsController
{
    use HandleTrait;
    ...
    {
        try {
            $this->handle(message: new RestoreProductionItemIngredientsCommand(
                productionId: $request->attributes->get(key: 'productionId'),
                itemId: $request->attributes->get(key: 'itemId'),
                restoredByUserId: RequestExtractor::getUserSessionId(request: $request),
            ));
