...
- **Las pantallas de detalle se maquetan con `<ds-split-view>`.** Una sola columna por debajo de 1000px y dos por encima: en `slot="side"` va el contenido de contexto (identidad, resumen, filtros, gráficas, calendario) y sin slot el contenido principal (listas, formularios largos, acciones).
  - La columna lateral es **sticky por defecto** (`sticky` es `true`): se queda fija con `top: var(--ds-space-5)` y, si no cabe, scrollea por dentro. Sólo se pasa `[sticky]="false"` cuando esa columna deba scrollear con la página, y `stickyTop` si hay que separarla más del borde.
  - **El skeleton de carga replica el mismo `ds-split-view`** (mismas `columns`, mismo reparto side/main) para que el layout no salte al llegar los datos.
